...
export async function checkDuplicateAgainstSaved(question, {excludeRunId = null} = {}) {
  const currentText = questionText(question);
  const best = {
    is_duplicate: false,
    similarity: 0,
    matched_run_id: null,
  ...
  for (const item of index) {
    ...
    if (passageId(question) && passageId(question) === passageId(candidateQuestion)) {
      continue;
    }

    const similarity = jaccardSimilarity(currentText, questionText(candidateQuestion));
