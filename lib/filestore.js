...
import {metadataQuestionCount} from "./question-count.js";
...
function summarizeUsageCalls(calls) {
  ...
  for (const call of Array.isArray(calls) ? calls : []) {
    ...
    if (!Object.hasOwn(usage, provider)) continue;
    usage[provider].prompt_tokens += Number(call.prompt_tokens || 0);
    usage[provider].output_tokens += Number(call.output_tokens || 0);
    usage[provider].total_tokens += Number(call.total_tokens || 0);
  }
  return usage;
}
...
function entryFromMetadata(runId, metadata, savedItem = {}) {
  ...
  return {
    ...
    question_count: metadataQuestionCount(metadata),
    ...
    token_usage: {
      question: summarizeUsageCalls(metadata?.ai_usage?.calls),
      explanation: summarizeUsageCalls(metadata?.explanation_review?.usage),
    },
    path: itemPath,
  };
}
